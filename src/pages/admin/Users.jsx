  ...
  const handleUserSubmit = async (userData) => {
    try {
      ...
      if (!selectedUser) {
        ...
        if (typeId === "3") {
          // Student: create user + student record immediately (no course picker)
          resetPendingStudentFlow();
          setCreationSaving(true);
          try {
            const createdUser = await createUser({
            ...
            // handle profile upload if necessary
            const photoToUpload =
              mergedCreate.ProfilePicture || mergedCreate.profilepicture || null;
            if (
              photoToUpload &&
              typeof photoToUpload === "string" &&
              photoToUpload.startsWith("data:")
            ...
            // build student payload and call createStudent
            const enrollmentDateValue =
              mergedCreate.EnrollmentDate ?? mergedCreate.enrollmentDate ?? null;

            const studentPayload = {
              UserID:
                createdUser.UserID ?? createdUser.id ?? createdUser.userID ?? createdUser.userId,
              RollNumber:
                mergedCreate.RollNumber ?? mergedCreate.IDNumber ?? mergedCreate.rollNumber ?? mergedCreate.idNumber ?? undefined,
              EnrollmentDate: enrollmentDateValue ?? undefined,
              CurrentGrade:
                mergedCreate.CurrentGrade ?? mergedCreate.Class ?? mergedCreate.currentGrade ?? mergedCreate.class ?? undefined,
              ParentName:
                mergedCreate.ParentName ?? mergedCreate.GuardianName ?? mergedCreate.parentName ?? mergedCreate.guardianName ?? undefined,
              ParentContact:
                mergedCreate.ParentContact ?? mergedCreate.GuardianPhone ?? mergedCreate.parentContact ?? mergedCreate.guardianPhone ?? undefined,
            };
            ...
            try {
              ...
              console.error("Failed to create student record:", studentErr);
              setFormError(studentErr?.message || "Student created but failed to save details");
            }
            ...
            if (createdStudent && typeof createdStudent === "object") {
              const studentIdCandidate =
                createdStudent.StudentID ?? createdStudent.studentID ?? createdStudent.studentId ?? createdStudent.id ?? null;
              if (studentIdCandidate !== null && studentIdCandidate !== undefined) {
                enrichedUser.StudentID = studentIdCandidate;
                enrichedUser.studentID = studentIdCandidate;
              }
